...
def validate(net, val_data, ctx, eval_metric):
    ...
    for batch in val_data:
        ...
        for x, y in zip(data, label):
            ...
            det_bboxes.append(bboxes.clip(0, batch[0].shape[2]))  # 让bboxes坐标为图片大小范围内
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    mbox_loss = gcv.loss.SSDMultiBoxLoss()  # compute loss in entire batch across devices
...
if __name__ == '__main__':
    ...
    # 为什么这里要有两个network, 也就是net 和 async_net
    ...
    train_data, val_data = get_dataloader(async_net, train_dataset, val_dataset, args.data_shape,
                                          batch_size, args.num_workers, ctx[0])
